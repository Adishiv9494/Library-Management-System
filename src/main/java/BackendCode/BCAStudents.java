...
@WebServlet("/BCAStudents")
@MultipartConfig(
    fileSizeThreshold = 1024 * 1024,
    maxFileSize = 1024 * 1024 * 10,
    maxRequestSize = 1024 * 1024 * 50
)
public class BCAStudents extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try (Connection conn = DriverManager.getConnection(dbURL, dbUser, dbPassword)) {
                String sql = "INSERT INTO bca_students (crn, name, course, contact) VALUES (?, ?, ?, ?)";
        ...
        out.println("<script>");
        out.println("Swal.fire({title: 'Import Completed', html: '<b>Success:</b> " + successCount + "<br><b>Duplicate:</b> " + duplicateCount + "', icon: 'success'}).then(()=>window.location.href='Report.jsp');");
        out.println("</script>");
