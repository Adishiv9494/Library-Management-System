...
if (rollNo == null || rollNo.trim().isEmpty() || email == null) {
    ...
    return; // Stop rendering the page if not logged in
}

// Generate initials for the profile picture fallback
String initials = "S";
...
try {
    ...
    try (PreparedStatement ps1 = conn.prepareStatement("SELECT COUNT(*) FROM book_issues WHERE crn = ?")) {
        ...
        try (ResultSet rs1 = ps1.executeQuery()) {
            if (rs1.next()) myIssuedCount = rs1.getInt(1);
        }
    } catch(Exception e) { /* Ignore */ }
    
    // Count 2: Available Books Count
    try {
        try (PreparedStatement ps2 = conn.prepareStatement("SELECT COUNT(*) FROM booksdata WHERE accession_no NOT IN (SELECT accession_no FROM book_issues)");
             ResultSet rs2 = ps2.executeQuery()) {
            if (rs2.next()) availableCount = rs2.getInt(1);
        }
    } catch (Exception e1) {
        try (PreparedStatement ps2 = conn.prepareStatement("SELECT COUNT(*) FROM booksdata WHERE accession_number NOT IN (SELECT accession_number FROM book_issues)");
             ResultSet rs2 = ps2.executeQuery()) {
            if (rs2.next()) availableCount = rs2.getInt(1);
    ...
    // Count 4: Downloaded E-Books strictly for this logged-in student
    try (PreparedStatement ps4 = conn.prepareStatement("SELECT COUNT(*) FROM downloaded_ebooks WHERE crn = ?")) {
        ...
        try (ResultSet rs4 = ps4.executeQuery()) {
            if (rs4.next()) downloadEBooksCount = rs4.getInt(1);
        }
    } catch(Exception e) { /* Ignore */ }

} catch(Exception e) {
    // Database connection issues silently ignored for UI rendering
} finally {
...
<head>
    ...
    <style>
        ...
        * { font-family: "Ubuntu", sans-serif; margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Make only the UL scrollable so the profile stays fixed at the bottom */
        .navigation ul { 
        ...
        /* Hover border curve style blending with --gray main background */
        ...
        /* EXACT Original Sidebar Profile Section */
        ...
        @media (max-width: 480px) { 
            ...
            .gallery-title { font-size: 1.5rem; } .slider-controls .dot { width: 10px; height: 10px; }
        }
    </style>
</head>
<body>
    <div class="container animate__animated animate__fadeIn">
        ...
        <div class="main">
            ...
            <div class="cardBox">
                <!-- Count 1: Issued to this student -->
                <div class="card" onclick="window.location.href='StudentIssuedBook.jsp'">
                    <div>
                ...
                <!-- Count 2: Currently available in library -->
                ...
                <!-- Count 4: Downloaded by this student -->
